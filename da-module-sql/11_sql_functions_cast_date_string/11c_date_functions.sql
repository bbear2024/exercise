...
SELECT DATE_TRUNC('minute', TIMESTAMP '2021-06-27 10:30:45') AS trunc_minute;
...
SELECT yearmonth,
		TO_TIMESTAMP(yearmonth::VARCHAR, 'YYYY-MM-DD HH:MI:SS'),
		monthly_count
FROM monthly_total
...
SELECT MAKE_INTERVAL(mins => actual_elapsed_time::FLOAT) AS ten_mins_interval from flights;
-- this will work
SELECT MAKE_INTERVAL(mins => actual_elapsed_time::INT) AS ten_mins_interval from flights;
